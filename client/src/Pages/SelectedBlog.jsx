...
export default function SelectedBlog() {
  ...
  const handleDeletePost = async () => {
    try {
      const res = await axios.delete(Server + "post/delete/" + blogId, {
  ...
  const fetchBlogComments = async () => {
    try {
      const res = await axios.get(Server + "comment/post/" + blogId);
      // console.log(res.data)
  ...
  const deleteBlogComments = async (id) => {
    try {
      const res = await axios.delete(Server + "comment/delete/" + id, {
        withCredentials: true,
      });
    ...
    <>
      <div className="flex justify-center my-4  items-center ">
        {loader ? (
          ...
          <div className="md:w-[50rem]   bg-white p-3 min-[401px]:p-7 max-[766px]:w-[95%]  md:p-12 flex flex-col gap-6 sm:gap-10 rounded-lg">
            ...
            {commnetKaru && (
              ...
              >
                <div className="flex justify-between px-3">
                  <div className=" max-[666px]:text-sm font-semibold text-slate-600">
                    Add Comment
                  </div>
            ...
            <div className="comment-section  w-full bg-slate-100 p-3 rounded  ">
              <div className="font-semibold text-slate-600  max-[666px]:text-sm">
                All Comments
              </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {authUser?._id === cm.userId && (
                        <div className="flex max-[506px]:text-base  text-xl gap-4 text-slate-700 cursor-pointer">
                          <MdDelete
                            onClick={() => deleteBlogComments(cm._id)}
                          />
                        </div>
                      )}
                    </div>
                    <div className="text-base max-[506px]:text-xs text-indigo-600">
                      {cm.comment}
                    </div>
